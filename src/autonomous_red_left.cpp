...
void sbot_run_red_left_auto() {
    ...
    sbot_jerry_start_y = SBOT_JERRY_START_RL_Y_BASE;
    sbot_zero_pose_and_sensors(0, 0, 0);  // Robot-relative frame
    sbot_print_pose("red left start");

    // Convert Jerry coords to robot-relative
    auto cluster = sbot_from_jerry(CLUSTER1_JERRY_X, CLUSTER1_JERRY_Y);
    auto midgoal = sbot_from_jerry(CENTER_MID_GOAL_JERRY_X, CENTER_MID_GOAL_JERRY_Y);
    auto matchload_retreat = sbot_from_jerry(RETREAT_POINT_JERRY_X, RETREAT_POINT_JERRY_Y);
    auto match_loader = sbot_from_jerry(LOADER_CONTACT_JERRY_X-2.2, LOADER_CONTACT_JERRY_Y);
    auto long_goal = sbot_from_jerry(LONG_GOAL_END_JERRY_X, LONG_GOAL_END_JERRY_Y);
    ...
    sbot_chassis->turnToHeading(313.5-90, 1000, {.maxSpeed = 60});
    sbot_chassis->waitUntilDone();
